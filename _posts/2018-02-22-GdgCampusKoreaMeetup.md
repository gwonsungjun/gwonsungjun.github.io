...
- 개발 프로세스
- 이슈발생 (할일) -> 유지보수||hotfix -> 스펙분석 -> 일정산정 -> 코딩 -> 커뮤니케이션 -> 문서화
...
- 코딩 ( 가독성이 좋은 코드 모듈화를 통한 재사용성 높은 코드 모듈화의 기준 Code Quality 과도한 추상화를 피하기 위한 인터페이스 설계 확장 가능한 설계 성능을 고려한 코드 유지보수가 용이한 코드 단일 책임 ...)
...
- 12월 파이썬 연말 세미나에서 로 발표 -> 주니어 개발자 되다!
    - 1. 미래의 나를 위한 기록을 잘하자 (개발일지, 블로그, 발표자료 등등) 2.Github 관리하자 3.정도를 걷자(뭐든 금방 이워지는 것은 없다)
